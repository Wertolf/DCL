...
from ...Locals.Display import CENTER, CENTERX, CENTERY, DISPLAY, WINWIDTH, WINHEIGHT
from ...Locals.Fonts   import BUTTON, COPYRIGHT, TITLE, USERNAME
from ...Locals.Paths   import TEMP
from ...Locals.Sounds  import HOMEBGM
...
def home():
    ...
    while True:
        ...
        try:
            ...
            user = loadUserData(username='__guest__')
        finally:
            updateTempUser(user)
        ...
        #welcome
        surf = Label(USERNAME, size=30, text='WELCOME,').get_surf()
        rect = surf.get_rect(left=0, top=0)
        DISPLAY.blit(surf, rect)
        surf = Label(USERNAME, size=30, text=user['username'].upper()).get_surf()
        rect = surf.get_rect(left=0, top=60)
        DISPLAY.blit(surf, rect)

        #title
        filename = TITLE
        surf = Label(filename, size=80, text='抽卡人生').get_surf()
        rect = surf.get_rect(top=160, centerx=CENTERX)
        DISPLAY.blit(surf, rect)

        #copyright
        filename = COPYRIGHT
        surf = Label(filename, size=30, text='Copyright%s2019 OurDreams Studio' % chr(169)).get_surf()
