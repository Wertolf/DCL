...
from ...Locals.Fonts   import ATTRIBUTE, BARS, BUTTON, INVENTORY, MARGIN, NEXTDAY, TIP
from ...Locals.Sounds  import GAMEBGM1, GAMEBGM2, ILLBGM
...
def game():
    ...
    buttonList, disableList, buttonDict = Buttons.buttonList, Buttons.disableList, Buttons.buttonDict

    if player['isNewGame'] and user['storyOn']: startStory()
...
def letters():
    ...
    #draw 'ATTRIBUTE'
    surf = Label(filename, size, text='ATTRIBUTE').get_surf()
    rect = surf.get_rect(top=180)
    DISPLAY.blit(surf, rect)

    #draw 'INVENTORY'
    surf = Label(filename, size, text='INVENTORY').get_surf()
    rect = surf.get_rect(top=360)
    DISPLAY.blit(surf, rect)
...
def items():
    ...
    for argDict in argList:
        kwargs = dict(filename= filename,
                      ...
                      antialias=False)
        if argDict['color']: kwargs['color'] = argDict['color']
