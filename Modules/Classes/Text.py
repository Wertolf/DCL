...
from ..Locals.Fonts   import LOADING, STORY, TIP
from ..Locals.Sounds  import TYPE
...
class Label:
    ...
    def get_surf(self, hlcolor=None):
        ...
        fontFormat = [self.filename, self.size]
        if hlcolor:
...
available keywords:
    ...
    if soundOn: TYPE.stop()

    if wait:
        skip = pause(text=waitText)
        return skip

def pause(filename=STORY, size=40, text='按空格键继续...'):
...
support multiline text.'''
    ...
    if mode == 'showLines':
        showLines(
            ...
            filename=TIP, size= size, dist= dist,
